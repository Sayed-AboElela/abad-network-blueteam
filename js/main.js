jQuery(document).ready(function ($) {
        ...
        const glowObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    entry.target.style.filter = 'drop-shadow(0 0 20px rgba(76, 239, 243, 0.3))';
    ...
    function initTimelinePulse() {
        ...
        if (!document.querySelector('#pulseStyles')) {
            ...
            style.textContent = `
                @keyframes pulse {
                    0%, 100% {
                        // box-shadow: 0 0 15px rgba(76, 239, 243, 0.8);
                        transform: scale(1);
                    }
                    50% {
                        // box-shadow: 0 0 25px rgba(76, 239, 243, 1), 0 0 40px rgba(76, 239, 243, 0.6);
                        transform: scale(1.2);
                    }
                }
            `;
    ...
    function initAnimatedTitles() {
        ...
        if (!document.querySelector('#titleGradientStyles')) {
            ...
            style.textContent = `
                .section-title {
                    background: linear-gradient(90deg, #4ceff3, #df5aff, #4ceff3);
